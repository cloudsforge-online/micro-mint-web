...
export default defineConfig({
  ...
  //
  // `/create` is the same string on localhost, on testnet, on mainnet and in a preview: it is a
  // fact about how the estate composes this surface's URLs, not about which estate serves it.
  //
  // TRAILING SLASH REQUIRED. vite joins `base` to an asset name by concatenation, so `/create`
  // emits `/createassets/index-a1b2.js` — a 404 for the bundle on every page, with a build that
  ...
  // ════════════════════════════════════════════════════════════════════════════════════════════
  // 5184 IS A VITE PORT, NOT THE REGISTRY'S `create` ENTRY, AND THE DIFFERENCE MATTERS HERE.
  //
  ...
  // Run mint with `PORT=4004`; the README says so in one line, and the finding is reported to
  // micro-ui, whose file the registry is.
  //
  // It is invisible in production, where this bundle and mint share `create.<apex>` and every
