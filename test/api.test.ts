...
describe('auth callback', () => {
  ...
  it('strips the code from the address bar BEFORE the redemption is sent', async () => {
    browser = installWindow('https://cloudsforge.online/create/reports?tab=1#cf_code=abc123&view=grid')
    ...
    assert.equal(browser.trace[0], 'replaceState:/create/reports?tab=1#view=grid')
    assert.ok(browser.trace[1]?.startsWith('fetch:'))

    // The rest of the fragment survives: an app may keep its own route there.
    assert.deepEqual(browser.replaced, ['/create/reports?tab=1#view=grid'])
  ...
  it('redeems at the route identity serves, on identity’s host', async () => {
    browser = installWindow('https://cloudsforge.online/create/#cf_code=abc123')
    stub = installFetch(identityRedemption, browser.trace)
  ...
  it('still strips the code when the redemption fails', async () => {
    ...
    browser = installWindow('https://cloudsforge.online/create/#cf_code=dead')
    stub = installFetch(() => json(400, { error: 'code expired' }), browser.trace)

    assert.equal(await bootstrapSession(), false)
    assert.deepEqual(browser.replaced, ['/create/'])
    assert.equal(getAccessToken(), null)
  })

  it('does nothing to a URL that carries no code', async () => {
    browser = installWindow('https://cloudsforge.online/create/overview#section-2')
    let calls = 0
    stub = installFetch(() => {
      calls += 1
      return json(200, {})
