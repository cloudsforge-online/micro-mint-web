 ...
 *
 * `test/mint.test.ts` proves that every route this bundle calls is really registered by micro-mint,
 * and really authenticates the way this client believes. To do that it has to read a file that a
 * DIFFERENT repository owns and edits, and that is the whole difficulty: any anchor naming a
 * POSITION in that file is a promise this repository cannot keep.
 *
 * micro-org#235 is the bill for that promise, and THIS repository wrote the worst line of it. The
 * suite cited `path:line`; micro-mint gained an erasure webhook and its routes moved; and the check
 * that the two routes called without a token make no `authenticate()` call sliced a handler body
 * from the recorded line 384, landed in the TAIL OF THE PREVIOUS HANDLER, and reported that
 * `POST /v1/tokens` does not authenticate. It does — `const principal = await authenticate(ctx,
 * deps)` is the FIRST STATEMENT of that handler in `mint/src/server.ts`. The test accused this
 * frontend of a defect that did not exist, which is a good deal worse than a test that merely
 * fails: a red run that names the wrong culprit gets somebody to change working code.
 *
 * ── A BYTE OFFSET IS THE SAME DEFECT IN A DIFFERENT UNIT ────────────────────────────────────────
 *
 * Replacing `lines[384]` with `source.slice(at, at + 900)` does not fix this; it re-anchors it.
 * "Nine hundred characters from a match" is not a function, it is a window that happens to contain
 * one today, and it fails in BOTH directions: RED when the construct grows past the window, for a
 * reason belonging to a different repository, and GREEN when the window overruns the construct and
 * the assertion is satisfied by the NEXT one. The second is the dangerous half. A guard that cannot
 * fail is worse than no guard, because it is believed.
 *
 * The same is true of a boundary written as "up to the next `define(`". That was this repository's
 * `bodyOf`, and `POST /v1/events` is the LAST route micro-mint registers — so the webhook's "body"
 * was every line from its registration to the end of the file, `ownedToken`, `authenticate` and all
 * two hundred and fifty lines of helpers included. The assertion that the webhook takes no bearer
 * was one edit in micro-mint away from failing, and it would have failed reading "mint-web is wrong
 * about the webhook". A terminator that can run off the end of the construct is not a boundary.
 *
 ...
 * mint dropped this route" from "the parser stopped understanding the file" without opening it.
 */
 ...
 *                `'POST /v1/tokens/:id/deploy'` or inside a template can never be mistaken for the
 *                one that closes a call.
 *
 ...
 * this suite asserts with. Without that, micro-mint could turn this repository red by reformatting
 ...
 * lines by a formatter without this repository concluding that micro-mint dropped a route. The
...
export function readServiceSource(file: string, text: string): ServiceSource {
  ...
  const functionBody = (name: string): string => {
    ...
    // Skip the RETURN TYPE, which may itself contain braces. The service helpers in this estate are
    // declared like `): Promise<{ principal: Principal; token: TokenRecord }> {`, and brace-matching
    // the first `{` after the parameters returns the OBJECT TYPE — a span with no code in it, over
    // which every `doesNotMatch` assertion passes and every `match` assertion fails for a reason
    // that has nothing to do with the service. The body is the first braced region that is not
    // immediately continued by type syntax.
    for (;;) {
