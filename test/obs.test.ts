...
describe('the envelope', () => {
  it('stamps the page the event came from', () => {
    installWindow('https://cloudsforge.online/create/reports?tab=1')
    const wrapped = event(1)
    assert.equal(wrapped.attributes['url'], 'https://cloudsforge.online/create/reports?tab=1')
    assert.equal(wrapped.route, '/create/reports', 'the route is the column Lantern groups on')
    assert.equal(
