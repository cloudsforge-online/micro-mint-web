 ...
 *     lock, wakes to find `paid`, and throws `OrderStateError` → **409** (`mint/src/server.ts:287-291`).
 *     Nobody is charged twice. But the 409 lands in this hook's `error`, so the screen renders "The
 *     payment did not go through" beside "Paid. The Shards have been debited". The `replayed: true`
 ...
 *   - DEPLOY. The enqueue is `onConflict: 'keep'` (`mint/src/server.ts:547-552`), so three clicks
