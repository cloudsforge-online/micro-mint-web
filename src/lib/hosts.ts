 ...
 *
 * This is the same shape as `admin` (registry 3002, `admin-api` binds 4014) and `emberkin`
 * (registry 3014, service binds 4100) — the third and fourth instance of a devPort that was an
 * allocation pretending to be a fact. It is NOT fixed with a literal port here: a hard-coded host
 * is a second, unversioned copy of the registry, and the copy is the one that goes stale.
 * `surfaces.test.ts` in micro-ui pins only the services whose port is distinctive, and mint's
 * 4000 is the service-template default that half the estate shares — so the registry cannot give
 * every one of them its bound port and the entry really is an allocation. What is missing is
 * anything that MAKES it true, so the README says `PORT=4004 pnpm dev`, in one line, next to the
 * citation. Reported to micro-ui.
 *
...
export const APP_NAME = 'mint-web'

/**
 * The base URL for this app's OWN API, which is `mint`.
 *
 * In production the SPA and `micro-mint` are the same origin — nginx serves the bundle, the
 * service serves `/v1` behind `create.<apex>` — so the base is the empty string and requests stay
 ...
 * `mint`, the account portal, Lantern — resolves one level too deep. The app still renders,
 * because unlike the operator console this one has a public page worth showing and nothing here
 * is a security boundary; but it says so, once, in the shell.
 */
