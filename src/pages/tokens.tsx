/**
 * Your launches. `GET /v1/tokens` — `mint/src/server.ts:417`.
 *
 * The service returns at most 100 rows (`listTokens(deps.sql, userSubject(userId), 100)`,
 * `server.ts:422`) and takes NO cursor: there is no pagination to offer, and pretending otherwise
 ...
 *
 * The `userId` query parameter the handler reads (`server.ts:420-421`) is not exposed by this app;
...
/** `mint/src/server.ts:422` passes 100 as the limit. */
        ...
        <>
          <div className="mw-tablewrap">
            <table className="mw-table">
              <caption className="mw-table__caption">
                ...
                (`order by created_at desc`, mint/src/tokens.ts:227). Select a row for its status,
          ...
          {orders.data.tokens.length >= SERVICE_LIMIT && (
            ...
            <p className="mw-note mw-note--warn" role="status">
              ...
              returns (mint/src/server.ts:422). It takes no page cursor, so older launches cannot be
